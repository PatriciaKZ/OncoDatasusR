...
#' print(plot)
#' }
plot_patient_count <- function(data,
                               method = c("APAC", "AIH"),
                               y_type = "number",
                               group = NULL,
                               first_year = NULL,
  ...
  if (method == "APAC") {
    ano <- ifelse(age_type == "age_diagnosis",
                  "ano_diagnostico",
                  "ano_atendimento")
    x_label <- ifelse(age_type == "age_diagnosis",
                      "Ano de diagnóstico",
                      "Ano de atendimento")
  } else {
    ano <- "ano_internacao"
    x_label <- "Ano de internação"
  }
  ...
  if (!is.null(group)) {
    ...
    if (group == "sex")
      ...
      group <- ifelse(method == "APAC", "cid_prim", "cid")
    else if (group == "procedure")
      group <- ifelse(method == "APAC", "tratamento", "proc_rea")
    else if (group == "database")
      group <- "Base"
    else if (group == "age_group") {
      data <- data %>%
  ...
  if (is.null(first_year))
    first_year <- min(data[[ano]], na.rm = TRUE)

  if (is.null(last_year))
    last_year <- max(data[[ano]], na.rm = TRUE)

  # Create summary tables
  APAC_plot <- data %>%
    dplyr::filter(.data[[age_col]] >= age_params[[age_group]]$min_age &
                    .data[[age_col]] <=
                    age_params[[age_group]]$max_age) %>%
    dplyr::filter(.data[[ano]] >= first_year &
                    .data[[ano]] <= last_year) %>%
    {
      if (!is.null(group))
        dplyr::group_by(., .data[[ano]], .data[[group]])
      else
        dplyr::group_by(., .data[[ano]])
    } %>%
    dplyr::summarise(n = dplyr::n_distinct(cod_paciente),
                     .groups = "drop") %>%
  ...
  if (y_type == "number") {
    ...
    y_label <- "Número de pacientes únicos"
  } else if (y_type == "percentage") {
    APAC_plot <- APAC_plot %>%
      dplyr::left_join(APAC_total, by = c(ano)) %>%
      dplyr::mutate(y_value = round((n / total) * 100, 1))
    y_label <- "% de pacientes únicos"
  }

  # Create the ggplot object

  g_plot <- ggplot2::ggplot(APAC_plot, ggplot2::aes(x = ano_x_value, y = y_value)) +
    ...
    linewidth = line_size) +
    ggplot2::geom_point(ggplot2::aes(color = if (!is.null(group))
      .data[[group]]
      else
        NULL), size = point_size) +
    ggplot2::theme_minimal() + {
      if (!is.null(group)) {
        ggplot2::scale_color_manual(
          values = setNames((scales::hue_pal())(length(
            unique(APAC_plot[[group]])
          )), unique(APAC_plot[[group]])),
          name = if (!is.null(legend_title))
            legend_title
          else
            group,
  ...
  if (include_label) {
    if (!is.null(group)) {
      ...
      APAC_plot <- APAC_plot %>%
        dplyr::mutate(nudge_y = if (!is.null(group))
          ny[match(.data[[group]], unique(APAC_plot[[group]]))]
          else
            0,
          nudge_x = if (!is.null(group))
            nx[match(.data[[group]], unique(APAC_plot[[group]]))]
          else
            0)
    ...
    g_plot <- g_plot + ggplot2::geom_text(
      data = APAC_plot,
      mapping = ggplot2::aes(label = y_value),
      color = text_color,
      position = ggplot2::position_nudge(x = APAC_plot$nudge_x, y = APAC_plot$nudge_y)
    )
  }
  ...
  if (!is.null(ylim)) {
    if (is.null(y_break)) {
      y_break <- if (y_type == "percentage")
        20
      else
        round(diff(range(APAC_plot$y_value)) / 5, 0)
    }
    g_plot <- g_plot + ggplot2::scale_y_continuous(
