...
# Check the directory where the file 'exported_data/apac_cervical_analysis.RData' is saved and adjust the path accordingly.
```

# 1. Introduction 

This report presents the results of the analysis of the count of patients treated with chemotherapy, hormone therapy, immunotherapy and/or radiotherapy for cervical cancer, based on data provided by DATASUS and using the methodology proposed by Ziegelmann et all (paper in *peer review*) and implemented by the `OncoDatasusR` package. The R code used to generate the results presented here is available in the package documentation (see vignette "cervical_cancer_apac_example").
...
patient_table %>%
  ...
  kable(
    align = c("l", "c"),
    col.names = c("", paste0("n = ", n_patients_final))
  ...
  kableExtra::group_rows("Clinical Stage", 12, 17) %>%
...
It is important to note that no records are excluded in this step. This analysis is only for descriptive purposes and to provide information on the quality of the data.

## 3.3. Count of Patients by Unique CNS Codes
...
## 3.4. Analysis of Potential Duplicates
...
table_key_var %>%
  kbl(col.names = c("Sex", "ZIP5", "Calculated birth year"), align = "c") %>%
  kable_paper("hover", full_width = F)
```
...
table_dup_cns %>%
  kbl(col.names = c("Number of CNS", "Patients", "Proportion"), align = "c") %>%
  kable_paper("hover", full_width = F)
```
