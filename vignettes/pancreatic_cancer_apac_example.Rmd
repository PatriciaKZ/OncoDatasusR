---
title: "Pancreatic cancer: APAC example"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Pancreatic cancer: APAC example}
...
This vignette shows how to use the `OncoDatasusR` package to analyze pancreatic cancer data. In this example, we will download and process data from High Cost Procedures Authorizations 
(APAC in Portuguese) of the Outpatient Information System (SIA) to count the number of patients with pancreatic cancer starting treatment in the Brazilian public health system.

This vignette was written to allow the reader to replicate the Pancreatic cancer Example (paper in *peer review*) without having to download data from DATASUS and to obtain identical results. 
...
with pancreatic cancer "C25", from "all" federation units in Brazil, that 
the name of the data files to be saved will start with "pancreas", which will be 
...
#>  Reading: datasus_files/AQ_AC_pancreas_2008_2023.csv
#>  ...
#>  Writing: datasus_files/AQ_pancreas_2008_2023_full.csv
...
#>  Content type 'unknown' length 4541 bytes
#>  Reading: datasus_files/AR_AC_pancreas_2008_2009.csv
#>  ...
#>  Writing: datasus_files/AR_pancreas_2008_2023_full.csv
#>  File saved successfully.
```
...
apac <- read_apac(
  read_bases = c("AQ", "AR"),
  aq_file = "AQ_pancreas_2008_2023_full.csv",
  ar_file = "AR_pancreas_2008_2023_full.csv",
  dir = NULL,
  first_year = 2008,
  last_year = 2023,
  file_code = "pancreas",
...
#>  Reading: datasus_files/AQ_pancreas_2008_2023_full.csv
#>  Reading: datasus_files/AR_pancreas_2008_2023_full.csv
#>  Writing: datasus_files/APAC_2008_2023_procedures.csv

str(apac, max.level = 1)
#>  List of 2
#>   $ APAC     :'data.frame':      254240 obs. of  15 variables:
#>   $ diag_list:List of 7
```

### 5.1.1. Extracted APACs
...
The table bellow shows the number of records before and after applying a filter to remove APACs with non-interested procedures. In this example we decided not to exclude any procedure 
from the analyses. However, it is recommended  that procedures that are not appropriate for the cancer under study be excluded due to potential errors.
...
The table bellow shows the number of APACs by procedure type. 
```{r table_procedure}
...
### 5.1.3. Count of patients by unique CNS codes
...
str(apac_sum)
#> List of 6
#>  $ APAC                  : tibble [35,306 × 27] (S3: tbl_df/tbl/data.frame)
#>  $ apac_per_patient      : tibble [90 × 2] (S3: tbl_df/tbl/data.frame)
...
#>  List of 2
#>   $ APAC_table      : tibble [35,306 × 30] (S3: tbl_df/tbl/data.frame)
...
#>   <int> <chr>                            <chr>                        <dbl>
#> 1     1 6924601e7b018a5624053daecb367dae C25_F_17760_20091022          1976
#> 2     2 de59a9367a3f108d6dd635d6a8fc96ce C25_F_17760_20091022          1977
#> 3     3 ffb5bcbe72617b92fd5ef7b80c9b37e3 C25_F_17760_20091022          1977
#> 4     4 6ecf367b4cea48e6e9071fbc9939060d C25_M_13500_20080102          1930
#> 5     5 93a651d61c2907caf590891ee9a011ea C25_M_13500_20080102          1930
#> 6     6 f3c73226f2294388ca5dd6c507f6d6ec C25_M_13500_20080102          1930
#> 7     7 329a1b31c0db0281dece15e207504eb3 C25_M_67000_20070712          1967
#> 8     8 5738046055d7aeabc88eafa2c97bffd3 C25_M_67000_20070712          1967
#> 9     9 74a2e96bad8acedd5ccd9c1a3d9c10fe C25_M_67000_20070712          1967
```
...
#>  All ok: each record appears in only one group.
#>  Processing time: 1.09959506988525
```

The function above returned a message confirming that each record belongs to a single group, indicating that the linkage variable is unique for each patient. As a result, you can proceed 
to the next step without requiring manual verification. However, if the function detects potential duplicates, you must manually review the records to confirm their status. 
For guidance on manual review scenario, refer to the "pancreatic_cancer_apac_example" vignette.

#### 5.3.4. Unify information of potential CNS duplicates
...
#>  List of 2
#>   $ APAC     : tibble [34,848 × 33] (S3: tbl_df/tbl/data.frame)
#>   $ cns_count: tibble [3 × 2] (S3: tbl_df/tbl/data.frame) 
```

#### 5.3.5. Unify information of potential CNS duplicates: quality information

The table bellow shows the number of duplicates CNS numbers by patient. 

```{r table_dup_cns}
table_dup_cns <- apac_merged$cns_count %>%
    mutate(p = paste0(format(round(n / sum(n) * 100, 2), nsmall = 2), "%")) %>%
    mutate(p = gsub("0.00\\%", "<0.01\\%", p))
```
...
apac_final <- prepare_apac(
  ...
  write = TRUE,
  output_dir = NULL,
  filename = "Base_APAC_pancreas_2008_2022_final.csv"
)
#> Writing: exported_data/Base_APAC_pancreas_2008_2022_final.csv

str(apac_final, max.level = 1)
#>  List of 2
#>   $ APAC_table: tibble [31,380 × 33] (S3: tbl_df/tbl/data.frame)
...
```{r var_linkage}
var_linkage <- round((nrow(apac_linkage$APAC_table) -
...
vignette "pancreatic_cancer_apac_report.Rmd" in the documentation folder of the package.

### 6.1. Descriptive table

The function below creates a table with the mean age and the number of patients with pancreatic cancer by sex, group age, geographic region of patient and clinical stage.
...
### 6.2. Plot of number of patients with pancreatic cancer by year of diagnosis

The number of patients with pancreatic cancer by treatment and year of diagnosis can be visualized using the function below. The y_type argument allows to choose whether to include 
...
save(
  ...
  n_rm_sex,
  n_patients_final,
  file = "exported_data/apac_pancreas_analysis.RData"
)
```
