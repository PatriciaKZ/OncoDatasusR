---
title: "Pancreatic cancer: AIH example"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Pancreatic cancer: AIH example}
...
This vignette was written to allow the reader to replicate the Pancreatic cancer Example (paper in *peer review*)
...
load("exported_data/aih_pancreas_analysis.RData")
```
...
This vignette shows how to use the package to analyze pancreatic cancer data. On this example, we will download and process data of hospital 
...
pancreatic cancer who underwent surgery in Brazilian public health system.
...
#> Reading: datasus_files/RD_AC_pancreas_2008_2023.csv
#> ...
#> Writing: datasus_files/RD_pancreas_2008_2023_full.csv
...
aih <- read_sih(
  dir = NULL,
  filename = "RD_pancreas_2008_2023_full.csv",
  first_year = 2008,
  last_year = 2023,
  proc_code = "04",
...
str(aih, max.level = 1)
#>  List of 2
#>   $ SIH      :'data.frame':      40466 obs. of  14 variables:
#>   $ diag_list:List of 5
```
...
aih_merged_zip_true <- merge_sih(
  ...
  write = TRUE,
  output_dir = NULL,
  filename = "Data_SIH_pancreas_2008_2022_withcep.csv"
)
#> Writing: exported_data/Data_SIH_pancreas_2008_2022_withcep.csv

str(aih_merged_zip_true, max.level = 1)
#> List of 2
#>  $ SIH_table: tibble [32,796 × 18] (S3: tbl_df/tbl/data.frame)
...
aih_merged_zip_false <- merge_sih(
  ...
  write = TRUE,
  output_dir = NULL,
  filename = "Data_SIH_pancreas_2008_2022_nocep.csv"
)
#> Writing: exported_data/Data_SIH_pancreas_2008_2022_nocep.csv

str(aih_merged_zip_false, max.level = 1)
#> List of 2
#>  $ SIH_table: tibble [21,430 × 18] (S3: tbl_df/tbl/data.frame)
#>  $ diag_list:'data.frame':      1 obs. of  15 variables:
```
...
see the vignette "pancreatic_cancer_aih_report.Rmd" in the documentation folder of the package.


### 6.1. Descriptive table

The function below creates a table with the mean age and the number of patients with pancreatic cancer by sex, group age, geographic region of patient.
...
The number of patients with pancreatic cancer by treatment and year of attendance can be visualized using the function below.
...
combined_plot <- plot_sih_zip_true$data %>%
  ...
  labs(
    x = "Year of hospitalization",
    y = "Number of patients",
    color = "Patient count method"
  ) +
  scale_x_continuous(
...
### 6.3. Plot by Top 4 Procedures

The plot below shows the number of patients with pancreatic cancer by procedure performed, considering scenario 1. For this example, we used the top 4 most frequent procedures.

First, we need to create a vector with the top 4 most frequent procedures to filter the data for plotting.

```{r top_procedures}
top_procedures <- aih_merged_zip_true$SIH_table %>%
  group_by(PROC_REA) %>%
  summarise(n = n()) %>%
  arrange(desc(n)) %>%
  slice(1:4) %>%
...
# Adjusting the procedure labels according to the SIGTAP table.

procedure_labels <- c(
  "0415020050" = "Sequential procedures",
  "0416040012" = "Bileo-Digestive anastomosis",
  "0416040128" = "Duodenopancreactomy",
  "0416040209" = "Multiple intra-abdominal biopsies"
)

plot_proced_sih_zip_true <- aih_merged_zip_true$SIH_table %>%
  filter(PROC_REA %in% top_procedures) %>%
  mutate(
    PROC_REA = recode(trimws(PROC_REA), !!!procedure_labels)
  ) %>%
  plot_patient_count(
    method = "AIH",
    y_type = "number",
...
```{r plot_procedure , eval=TRUE, fig.width = 7, fig.height = 3.5, fig.align = "center"}	
plot_proced_sih_zip_true
```
...
save(
  ...
  plot_sih_zip_false,
  combined_plot,
  file = "data/exported_data/aih_pancreas_analysis.Rdata"
)
```
